...
<template>
  <section @dragstart="false" class="c-avatar-cutter">
    <div class="mask"></div>
    <div class="container g-popup-box">
      <p class="title">选择头像</p>
      <div class="content">
        <div :class="{'c-left--doing':imgURL}" class="c-left">
          <div :style="{
        ...
        <div class="c-right">
          <div class="preview">
            <p>预览</p>
            <canvas :class="{'canvas--doing':imgURL}" height="190" ref="$canvas" width="190">></canvas>
          </div>
          <el-button class="btn-upload" type="primary">{{!imgURL ? '上传图片' : '重新上传'}}</el-button>
          <input @change="fileChange" accept="image/*" type="file"/>
        </div>
      </div>
      <div class="c-btn-group">
        <el-button @click="onCancel" class="btn-cancel">取消</el-button>
        <el-button :class="{'btn-enter--disable':!imgURL}" @click="onEnter" class="btn-enter" type="success">确定
        </el-button>
      </div>
    </div>
  </section>
      ...
      onEnter() {
        if (this.imgURL) {
          if (this.returnType === 'url') {
            ...
            this.$emit('enter', this.base64ToBlob(this.$refs.$canvas.toDataURL()))
          }
        } else {
